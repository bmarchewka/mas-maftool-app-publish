...
logging.info(f"Temporary directory ensured at: {tmp_dir}")
...
workspace_id = profile_data["workspaces"][0]["metadata"]["labels"]["mas.ibm.com/workspaceId"]
...
logging.info(f"Application {refresh_app_data.get("appName")} download status: {refresh_app_data.get("downloadStatus")}")
...
zip_output_path = os.path.join(tmp_dir, f"{app_id}.zip")
...
# -------------------------------------------------------------------------------------------
# 7. Verify publish status.
# -------------------------------------------------------------------------------------------
logging.info("-07- Verifying publish status")
...
logging.info(f"Final publish status: {publish_status}")